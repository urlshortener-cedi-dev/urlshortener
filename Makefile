...
# cedi.dev/urlshortener-bundle:$VERSION and cedi.dev/urlshortener-catalog:$VERSION.
IMAGE_TAG_BASE ?= cedi.dev/urlshortener
...
IMG ?= ghcr.io/urlshortener-cedi-dev/urlshortener:develop
...
manifests: controller-gen ## Generate WebhookConfiguration, ClusterRole and CustomResourceDefinition objects.
	$(CONTROLLER_GEN) rbac:roleName=urlshortener-role crd webhook paths="./..." output:crd:artifacts:config=config/crd/bases
...
.PHONY: docker-push
docker-push: ## Push docker image with the urlshortener.
	docker push ${IMG}
...
deploy: manifests kustomize ## Deploy controller to the K8s cluster specified in ~/.kube/config.
	cd config/urlshortener && $(KUSTOMIZE) edit set image controller=${IMG}
